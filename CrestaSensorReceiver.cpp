...
byte Cresta::SensorReceiver::halfBit = 0;
word Cresta::SensorReceiver::clockTime;
boolean Cresta::SensorReceiver::isOne;
unsigned long Cresta::SensorReceiver::lastChange=0;
Cresta::SensorReceiverCallback Cresta::SensorReceiver::callback;
byte Cresta::SensorReceiver::data[14];
byte Cresta::SensorReceiver::packageLength;
word Cresta::SensorReceiver::duration;
boolean Cresta::SensorReceiver::enabled;

void Cresta::SensorReceiver::init(int8_t interrupt, Cresta::SensorReceiverCallback callbackIn) {
  callback = callbackIn;
...
void Cresta::SensorReceiver::interruptHandler() {	
	if (!enabled) {
		return;
	}
...
void Cresta::SensorReceiver::reset() {
	halfBit = 1;
	clockTime = duration >> 1;
	isOne = true;
}

boolean Cresta::SensorReceiver::decryptAndCheck() { 
	byte cs1,cs2,i; 
...
byte Cresta::SensorReceiver::secondCheck(byte b) { 
  byte c;
...
void Cresta::SensorReceiver::enable() {
	halfBit = 0;
	enabled = true;
}

void Cresta::SensorReceiver::disable() {
	enabled = false;
}

void Cresta::SensorReceiver::decodeThermoHygro(byte *data, byte &channel, byte &randomId, int &temp, byte &humidity) {
	channel = data[1] >> 5;
